            ...
            foreach (array_keys($params['inventory_data']) as $itemtype) {
                if (in_array($itemtype, $availaibleItemType)) {
                    ...
                    if ($itemtype == Computer::getType()) {
                        ...
                        $db_info = new PluginFusioninventoryInventoryComputerComputer(); /** @phpstan-ignore-line */
                        if ($db_info->getFromDBByCrit(['computers_id' => $items_id])) { /** @phpstan-ignore-line */
                            $arrayinventory = unserialize(gzuncompress($db_info->fields['serialized_inventory'])); /** @phpstan-ignore-line */
